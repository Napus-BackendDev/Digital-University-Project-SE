...
const getRoleTitleText = function (role) {
  ...
  if (Array.isArray(role?.title)) {
    return role.title.map((item) => String(item?.value || '')).join(' ').toLowerCase();
  }
  return String(role?.title || role?.code || role || '').toLowerCase();
};
...
const buildUserFormsMatchCondition = function ({ userId, organizationId, isAdmin }) {
  ...
  if (userOID) {
    ...
    orConditions.push({ "collaborator.user": userOID });
...
const canUserSeeListedForm = ({ doc, targetUserId, isAdmin }) => {
  ...
  const isControllerFromCollaborator = Array.isArray(doc?.collaborator) && doc.collaborator.some((item) => {
    return String(item?.user?._id || item?.user || '') === userIdStr;
  });
  const isControllerFromControll = Array.isArray(doc?.controll) && doc.controll.some((item) => {
    return String(item?.user?._id || item?.user || '') === userIdStr;
  });
  ...
  const isPrivileged = isCreator || isControllerFromCollaborator || isControllerFromControll || isAllowedUser;
